...
# csv 불러오기
concept_mapping_df = pd.read_csv("./csv/concept_mapping_reduce.csv")
exercise_concept_df = pd.read_csv("./csv/exercise_concept.csv")
student_exercise_df = pd.read_csv("./csv/student_exercise_OT.csv")
...
join_df = join_df[["student_id", "exercise_no", "score", "option_score", "concept_no", "mapped_concept_no", "Categories_mapped_concept_no"]]
print(join_df.head())
print(len(join_df))
...
concept_df = join_df["Categories_mapped_concept_no"].drop_duplicates().dropna(axis=0)
...
for exer_id in exercise_list:
    ...
    knowledge_code = int(concept_mapping_df.loc[concept_mapping_df["concept_no"] == concept_no]["Categories_mapped_concept_no"].values[0])
...
option_list = list(join_df["option_score"])

for student_id, exer_id, score, option in zip(student_id_list, exercise_no_list, score_list, option_list):
    log_data_dict[student_id]["logs"].append({"exer_id": exercise_no_dict[exer_id], "score":score, "option":option, "knowledge_code": [concept_no_dict[exer_id]]})
